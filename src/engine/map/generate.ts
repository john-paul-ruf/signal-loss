...
export function generateMap(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt = attempt + 1) {
    ...
    const gateCtx: GateContext = { tunables, archetype: chosen };
    const gateCtxWithOptions: GateContext = {
      ...gateCtx,
      ...(options.gateCellSize !== undefined ? { cellSize: options.gateCellSize } : {}),
      ...(options.gateWallIndexCellSize !== undefined ? { wallIndexCellSize: options.gateWallIndexCellSize } : {}),
      ...(options.gateMeasureOptions !== undefined ? { measureOptions: options.gateMeasureOptions } : {}),
    };
    const report = runPlayabilityGate(map, gateCtxWithOptions);
    if (report.passed) {
