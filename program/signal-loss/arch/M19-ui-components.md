...
- `PlaybackTransport` — truthful playing/paused/complete, cursor, speed, step, and skip state; reduced motion exposes previous/next/skip and focusable information-complete event cards.
- `attack-model.ts` — pure attack-presentation boundary. It replaces positions in an app-only hypothetical state with the human `PublicState` projection, calls engine `exchangePreview()` separately for normal and called rows, routes exact-hit targets, guards pool edits, and derives committed-human playback spend. Ghosts use only last-confirmed public positions and remain labeled `POSITION UNCONFIRMED`.
...
### Result components — `./src/app/components/result/` (`complete-match-loop` SESSION-05)

- `ResultSummary` renders the complete semantic summary, accessible exact pool rows, CSS/SVG efficiency plot, visible/copyable roster strings, typed clipboard feedback, and recovery/rematch actions.
- `result-actions.ts` owns copy, same-seed rematch, new-seed rematch, and build-zone return service boundaries; all match values remain transient.
...
| Setup / result | `./src/app/components/setup/` — shipped by `match-setup-route` SESSION-04; `./src/app/components/result/` — shipped by `complete-match-loop` SESSION-05 |
...
- **Combat truth boundary:** UI attack matrices and legality never duplicate engine formulas. Both preview rows come from `exchangePreview()` over public positions; exact authoritative hidden coordinates never enter range/LOS presentation.
...
| 2026-08-29 | `complete-match-loop` SESSION-02 shipped truthful playback transport/log controls; SESSION-03 shipped public-safe engine-backed attack preview and guarded pool/keyboard controls; SESSION-05 shipped the complete result summary and copy/rematch action surfaces. |
