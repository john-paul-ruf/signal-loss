...
- `MatchScreen` creates a `MatchStore` instance per mount, consumes the shared flow-store launch payload once after resolving the catalog, and switches modes. An absent or rejected payload renders a `#/setup` recovery link rather than a playable fallback. One match-lifetime controller owns deployment, movement, and attack AI: every committed phase identity starts four real worker decisions, while phase change/unmount cancels and disposes the prior family and AI slot writes do not restart a run.
...
- `AttackMode` — exact-hit target routing plus `AttackLedger`/engine-backed `ExchangeCard`s, guarded called/posture pool edits, pointer and keyboard controls, and truthful committed-spend display during playback.
- `PlaybackMode` — event-prefix-driven board/log presentation with pause/resume, step, 1×/2×/4×, skip, zero-event completion, and information-complete reduced-motion cards.
- `ResultMode` — derives exactly one `MatchResultSummary` from final state, launch snapshot, human squad, and complete history; writes it directly to the app-lifetime flow store before `#/result` navigation. Derivation failure remains visible and does not navigate.
...
### Result route — `complete-match-loop` SESSION-05

- `./src/app/screens/result/route.tsx` self-registers id `"match-result"` at `#/result`; `ResultScreen` consumes the transient authoritative summary and offers copy, same-seed/new-seed rematch, and build-zone return.
- Missing transient data renders `MATCH SUMMARY UNAVAILABLE` with setup/build recovery rather than synthesizing a result. Rematch work is cancellable and never persists progression or result state.
...
| Result | `./src/app/screens/result/` | shipped and verified by `complete-match-loop` SESSION-05 |
...
- **Result handoff is direct and transient.** `ResultMode` writes the authoritative derived summary into the app-lifetime flow store before navigation. The result screen never reconstructs standings/accounting and never fabricates recovery data.
...
| 2026-08-29 | `complete-match-loop` SESSION-01 extended the match-lifetime AI controller through movement/attack; SESSION-02/03 delivered event-true playback and honest combat; SESSION-05 replaced the obsolete DOM-event result handoff with direct transient flow state and shipped the recoverable `#/result` route/rematch lifecycle. |
