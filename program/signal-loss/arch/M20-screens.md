...
- `#/setup` — needs the typed mapgen worker client at `./src/app/bridge/mapgen-client.ts` (worker entry `./src/workers/mapgen.worker.ts`, protocol `./src/workers/protocol.ts`) and must write `MatchLaunchConfig` (extended with `aiRosters`) to the core flow store on DEPLOY. Fully unstarted.
...
| Setup / result | `./src/app/screens/setup/`, `./src/app/screens/result/` | unstarted — pending SESSION-07 retry |
