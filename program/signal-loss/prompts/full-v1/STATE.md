...
| 06 | Release Content, Headless Batteries, and CI Gates | M01, M02, M16, M22 | `./data/catalog.chassis.json`<br>`./data/catalog.mounts.json`<br>`./data/catalog.commanders.json`<br>`./data/catalog.prebuilts.json`<br>`./data/tunables.json`<br>`./data/map.archetypes.json`<br>`./harness/**`<br>`./tests/harness/**`<br>`./docs/verification/**`<br>`./.github/workflows/ci.yml` | done | 6/6 | 2026-08-28 | Delivered M02 release content (7 chassis / 11 mounts / 4 commanders / 8 prebuilts / all-25 tunables / 7 archetypes, catalogHash=18a634daecb23aef tunablesHash=81071539e5673d96), M16 headless harness (CLI + 4 batteries + all aggregator over the engine facade), M22 harness self-tests (55 new tests, 664 total passing), and M01 blocking CI at .github/workflows/ci.yml. Every prebuilt validates through validateCatalogPrebuilts; every FR-11 archetype accepts at least 75% of seeds; determinism battery confirms REPLAY / FOLD / PERMUTATION identity; costing battery reports full budget-ladder tractability; behavior battery is honest about which FR-23 checks pass hard vs. report information-only at Session 06's initial weight tuning. 6/6 checkpoints committed. |
| 07 | Build Zone, Setup, Codex, and Result Surfaces | M02, M07, M14, M17, M19, M20, M22 | `./src/app/bridge/mapgen-client.ts`<br>`./src/app/store/build/**`<br>`./src/app/components/build/**`<br>`./src/app/components/setup/**`<br>`./src/app/components/result/**`<br>`./src/app/screens/boot/**`<br>`./src/app/screens/build/**`<br>`./src/app/screens/codex/**`<br>`./src/app/screens/setup/**`<br>`./src/app/screens/result/**`<br>`./tests/app/build/**`<br>`./tests/e2e/build/**` | blocked | 2/5 | 2026-08-28 | Retry 1 returned no handoff JSON; two checkpoint commits remain authoritative. Session-end residual `ed7b664` preserves uncommitted in-lease work. |
| 08 | Match Shell, Board, Plotting, and Playback | M09, M10, M11, M15, M17, M18, M19, M20, M22 | `./src/app/bridge/ai-client.ts`<br>`./src/app/store/match/**`<br>`./src/app/board/**`<br>`./src/app/components/match/**`<br>`./src/app/screens/match/**`<br>`./tests/app/match/**`<br>`./tests/e2e/match/**` | done | 6/6 | 2026-08-28 | Delivered M17 (bridge + match store), M18 (three-layer canvas board), M19 (match components), M20 (match screens): typed AI worker client with cancellation and id multiplexing; partitioned match store (engine slice + human drafts + AI slots + selection + playback + presentation) with drafts NEVER on MatchState (asserted structurally); persistent match shell (phase header, trace timeline, pool ledger, squad rail, inspector, round log, command bar, rules drawer); three-layer canvas (terrain/field/overlay) with one shared camera and arithmetic hit-testing; deployment/movement/attack/playback/result modes; freehand path simplification + engine-authoritative legality; exchange card reading engine exchangePreview; event-only playback with reduced-motion event-card parity (compile-time exhaustiveness); rules drawer opened by F1/? and dismissed by Escape; MatchResultPayload handoff via DOM custom event. 6/6 checkpoints committed; 57 new session-08 tests pass alongside the existing suite (605 stable tests); typecheck + lint + build all clean; static/offline CSP `connect-src 'none'` preserved. |
...
- **Status:** done
- **Checkpoint:** 6/6
- **Notes:** Delivered M02 release content (7 chassis / 11 mounts / 4 commanders / 8 prebuilts / all-25 tunables / 7 archetypes, catalogHash=18a634daecb23aef tunablesHash=81071539e5673d96), M16 headless harness (CLI + 4 batteries + all aggregator over the engine facade), M22 harness self-tests (55 new tests, 664 total passing), and M01 blocking CI at .github/workflows/ci.yml. Every prebuilt validates through validateCatalogPrebuilts; every FR-11 archetype accepts at least 75% of seeds; determinism battery confirms REPLAY / FOLD / PERMUTATION identity; costing battery reports full budget-ladder tractability; behavior battery is honest about which FR-23 checks pass hard vs. report information-only at Session 06's initial weight tuning. 6/6 checkpoints committed.
- **Follow-up:** For Session 03 (map generation): dense-grid generator needs a post-processing pass that punches holes in enclosed pockets larger than the tolerance, OR spacing needs to be a tunable Session 06 can override without changing generator internals. For Session 01 / DB owner: MIN_POCKET validator max of 1<<26 fx² is too small for BOARD_SIZE >= 48 grid maps — either raise the ceiling or accept the dense-grid follow-up above. For Session 06 self-follow-up: (a) test:determinism / playability / behavior / costing script definitions should be updated to point at tests/harness (owner: Session 01 owns package.json), (b) release AI weights need iteration in the behavior direction — increase traceSafetyBonus / traceExposurePenalty, or loosen TRACE_INTERVAL / TRACE_FIRST_ROUND, so trace-death rate drops below TRACE_DEATH_CEILING. For Sessions 07/08: cross-runtime determinism hooks in via runDeterminismBattery({crossRuntimeHashes: {chromium: [...], firefox: [...], webkit: [...]}}); e2e tests should record terminal hashes for every seed and hand them to the CI cross-browser job. Do NOT reuse RNG stream labels 'roster.squad<N>' or 'ai.squad<N>.deploy/move/attack' from the runner — they are stable per Session 04/05 handoff.
...
- **Checkpoint:** 2/5
- **Delegated to:** enso
- **Notes:** CP1 (boot #/ + codex #/codex) and CP2 (collection #/build: persistence, share import/export, storage banners) delivered and committed; CP3 composer, CP4 setup+mapgen, CP5 result+e2e remain.
- **Follow-up:** Resume at CP3. (a) The composer route is expected at `#/composer` (already linked from boot and the collection nav) with the design.md §5.2 locked keyboard map (Tab regions, arrows in lists, Enter mount/select, Backspace unmount, C commander, / search); reuse DialStatGrid for the commander before/after delta overlay and `applyCommanderType` from the engine. (b) CP4 setup `#/setup` + CP5 result `#/result`; setup needs the typed mapgen worker client at `./src/app/bridge/mapgen-client.ts` (worker entry is `src/workers/mapgen.worker.ts`, protocol `src/workers/protocol.ts`) and must write MatchLaunchConfig to the core flow store on DEPLOY. (c) Consume `resolveCatalog()` from `src/app/store/build/catalog.ts` everywhere; do not re-resolve. (d) `outcomeFromDecode` (share.ts) is the pure decode->UI map; UNKNOWN_ENTRY/VERSION_UNSUPPORTED can't be produced via the public encoder (it validates codes), so they are unit-tested on the pure mapping, not via crafted strings. (e) e2e specs under tests/e2e/build/*.spec.ts are authored but unrun pending a green cross-lease build; they use the Playwright preview webServer on 5173.
- **Blocked reason:** context exhausted at ckpt 2 — checkpoints 3 (keyboard composer), 4 (setup + deterministic mapgen worker client), and 5 (result + e2e + visual parity) not started. Both landed checkpoints are complete, verified, and committed; stopping rather than leaving an uncommittable partial checkpoint 3.
- **Accessibility notes:** Sortable headers use th[aria-sort] + button controls (keyboard Enter/Space); dial disclosure uses aria-expanded/aria-controls buttons; storage/import banners use role=status\|alert with aria-live; forms/inputs carry labels (sr-only where visual label is implicit); global :focus-visible ring from styles.css applies. Full keyboard-only + axe runs are authored as Playwright specs, unrun pending a green cross-lease build.
- **Evidence:** [{"shot":"structural: tests/app/build/boot.test.tsx","note":"boot renders real catalog counts+hash (not mock 0.4.1), contract line, five squad tags, three entry hrefs, and the 1280x720 statement"},{"shot":"structural: tests/app/build/codex.test.tsx","note":"codex renders every chassis/commander from the catalog, aria-sort headers, fixed-point-formatted dial values, and the permanent resolution-truth contract line"},{"shot":"structural: tests/app/build/collection-view.test.tsx","note":"collection renders pinned prebuilts + DUPLICATE-TO-EDIT, legal banner + re-importable SL1 share textarea, persistent import panel, and persistence-unavailable + armed-corrupt-reset banners"},{"shot":"screenshots pending","note":"live 1440x900 / 1280x720 capture deferred: the preview build is gated red by concurrent Session-08 match/* breakage; the authored e2e/axe specs must be run by the final verification session once both leases are green"}]

### SESSION-07 Retry 1

- **Status:** blocked
- **Checkpoint:** 2/5
- **Notes:** Failed to parse tool arguments: JSON Parse error: Property name must be a string literal
- **Blocked reason:** no handoff JSON; see result file
- **Result:** `./.forge/results/SESSION-07.result.md`
- **Follow-up:** Two checkpoint commits remain authoritative. Jikijitsu committed in-lease residual `ed7b664` after collection; resume at CP3 only under a fresh human-authorized retry, preserving the prior handoffs as history.

### SESSION-08

- **Status:** done
- **Checkpoint:** 6/6
- **Notes:** Delivered M17 (bridge + match store), M18 (three-layer canvas board), M19 (match components), M20 (match screens): typed AI worker client with cancellation and id multiplexing; partitioned match store (engine slice + human drafts + AI slots + selection + playback + presentation) with drafts NEVER on MatchState (asserted structurally); persistent match shell (phase header, trace timeline, pool ledger, squad rail, inspector, round log, command bar, rules drawer); three-layer canvas (terrain/field/overlay) with one shared camera and arithmetic hit-testing; deployment/movement/attack/playback/result modes; freehand path simplification + engine-authoritative legality; exchange card reading engine exchangePreview; event-only playback with reduced-motion event-card parity (compile-time exhaustiveness); rules drawer opened by F1/? and dismissed by Escape; MatchResultPayload handoff via DOM custom event. 6/6 checkpoints committed; 57 new session-08 tests pass alongside the existing suite (605 stable tests); typecheck + lint + build all clean; static/offline CSP `connect-src 'none'` preserved.
- **Follow-up:** For Session 07 result-screen work: the match store dispatches `signal-loss:match-result` DOM CustomEvent with a `MatchResultPayload` at match complete; subscribe from the core flow store. The AI-worker client factory is a parameter — Session 07's setup screen should share the same factory with the match store to keep one worker pool. The current `boot(config, catalog, map)` seeds all five squads with the human roster (Session 07's saved-roster surface hasn't shipped AI-roster generation into the launch payload yet); extend `MatchLaunchConfig` with `aiRosters: [Roster, Roster, Roster, Roster]` and thread through boot(). The reach envelope samples a circle from chassis footprint + current dial allowance — walls not respected in the outline (deliberate; legalMovePlot refuses wall-crossing at commit). Session 07 owns the derivePayload roster hydration path in ResultMode — it currently emits an empty constructs array under the roster shape; hydrate from the persisted roster keyed by rosterId when the result screen mounts. The AttackLedger + ExchangeCard use `select` + `button` for now (component-tree renderable without jsdom); Session 06's e2e can validate the exact 2x2 integers via `getByTestId('cell-*')`. Board canvas requires a browser with ResizeObserver + Canvas 2D; jsdom would need mocks — deferred to Playwright.
